# Lynx Consumer Staples Analysis -- API Reference

Public Python API for the `lynx_staples` package (v1.0).
...
### Analysis (`lynx_staples.core.analyzer`)
...
Run a complete fundamental analysis for a consumer staples company.
...
|---|---|---|---|
| `identifier` | `str` | required | Ticker symbol (`PG`), ISIN (`US7427181091`), or company name (`Procter & Gamble`). |
...
```python
from lynx_staples.core.analyzer import run_full_analysis

report = run_full_analysis("PG")
print(report.profile.name)       # "Procter & Gamble Co"
...
```python
from lynx_staples.core.analyzer import run_progressive_analysis

def on_progress(stage: str, report):
    print(f"Stage complete: {stage}")

report = run_progressive_analysis("KO", on_progress=on_progress)
```

---

### Conclusion (`lynx_staples.core.conclusion`)
...
```python
from lynx_staples.core.conclusion import generate_conclusion
...
## Data Models (`lynx_staples.models`)
...
| `CompanyStage` | `GRASSROOTS` ("Early Stage / Pre-Profit"), `EXPLORER` ("Emerging Staples Brand"), `DEVELOPER` ("Scaling Staples Operator"), `PRODUCER` ("Mature Defensive Operator"), `ROYALTY` ("Brand Licensor / Asset-Light") |
| `Segment` | `PACKAGED_FOOD`, `BEVERAGES_NA`, `BEVERAGES_ALC`, `TOBACCO`, `HOUSEHOLD_PRODUCTS`, `PERSONAL_PRODUCTS`, `FOOD_RETAIL`, `HYPERMARKET`, `DRUG_RETAIL`, `AGRIBUSINESS`, `OTHER` |
...
| `sector` | `Optional[str]` | Sector (e.g. `"Consumer Defensive"`). |
| `industry` | `Optional[str]` | Industry (e.g. `"Packaged Foods"`). |
...
| `primary_segment` | `Segment` | Primary consumer staples sub-segment (default `OTHER`). |
...
Traditional and consumer-staples-specific valuation ratios.
...
`dilution_ratio`, `production_growth_yoy` (reused as volume / unit growth proxy),
...
## Classification Helpers (`lynx_staples.models`)
...
Identify primary consumer staples sub-segment (Packaged Food, Non-Alcoholic
Beverages, Alcoholic Beverages, Tobacco, Household Products, Personal Care,
Food Retail, Hypermarkets & Super Centers, Drug Retail, Agribusiness) from
description and industry text using keyword frequency scoring.
...
## Metrics Calculator (`lynx_staples.metrics.calculator`)
...
| `calc_business_quality(profitability, growth, solvency, share_structure, statements, info, tier, stage)` | `BusinessQualityIndicators` | Composite 0-100 score across brand strength, unit economics, financial position, management alignment, and capital discipline. Defensive-sensitivity tagging. |
...
## Relevance System (`lynx_staples.metrics.relevance`)
...
operator maturity stage is the primary analytical axis for consumer-staples operators.
...
## Storage (`lynx_staples.core.storage`)
...
## Ticker Resolution (`lynx_staples.core.ticker`)
...
Accepts:
- **Ticker symbols:** `PG`, `KO`, `MO`
- **ISIN codes:** `US7427181091` (12-character format)
- **Company names:** `"Procter & Gamble"`, `"Coca-Cola"`
...
```python
from lynx_staples.export import ExportFormat
```

---

## Sector Insights (`lynx_staples.metrics.sector_insights`)
...
Available sectors: `"Consumer Defensive"`, `"Consumer Staples"`.
...
Available industries: `"Packaged Foods"`, `"Non-Alcoholic Beverages"`,
`"Alcoholic Beverages"`, `"Tobacco"`, `"Household Products"`,
`"Personal Products"`, `"Food Retail"`, `"Hypermarkets & Super Centers"`,
`"Drug Retail"`, `"Agribusiness"`.
...
report = run_progressive_analysis("KO", on_progress=progress_handler)
```
...
```python
from lynx_staples.metrics.relevance import get_relevance
from lynx_staples.models import CompanyTier, CompanyStage, Relevance
...
from lynx_staples.core.analyzer import run_full_analysis
from lynx_staples.export import ExportFormat, export_report

report = run_full_analysis("PG")

# Export as HTML (default path: data/PG/report_<timestamp>.html)
