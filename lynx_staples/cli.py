"""Command-line interface for lynx-staples (Consumer Staples Analysis)."""
...
from lynx_staples import __author__, __author_email__, __license__, __version__, __year__, SUITE_LABEL


def build_parser():
    parser = argparse.ArgumentParser(
        prog="lynx-staples",
        description="Lynx Consumer Staples Analysis — fundamental analysis tool for consumer staples companies.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  lynx-staples -p PG                            Procter & Gamble — household products mega-cap\n"
            "  lynx-staples -p KO                            Coca-Cola — non-alcoholic beverage leader\n"
            "  lynx-staples -p WMT                           Walmart — hypermarket / super center\n"
            "  lynx-staples -p MO --refresh                  Altria — force fresh data\n"
            '  lynx-staples -p "Procter & Gamble"            Search by name\n'
            "  lynx-staples -p -i                            Interactive mode\n"
            "  lynx-staples -t COST                          Testing mode\n"
        ),
    )
...
def run_cli():
    ...
    if "--b2m" in sys.argv:
        ...
        from lynx_staples.easter import rich_rocket, rich_fortune
        c = Console()
        rich_rocket(c)
        rich_fortune(c)
        return
    ...
    if "--about" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from lynx_staples import get_about_text
    ...
    if "--explain" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from rich.table import Table
        from lynx_staples.metrics.explanations import get_explanation, list_metrics
    ...
    from lynx_staples.core.storage import set_mode, is_testing
    ...
    if args.interactive:
        from lynx_staples.interactive import run_interactive; run_interactive(); return
    if args.tui:
        from lynx_staples.tui.app import run_tui; run_tui(); return
    if args.gui:
        from lynx_staples.gui.app import run_gui; run_gui(args); return
    if args.search:
    ...
    from lynx_staples.display import display_report_stage
    try:
        ...
        if args.export:
            from pathlib import Path; from lynx_staples.export import ExportFormat, export_report
...
def _cmd_list_cache(con):
    from rich.table import Table; from lynx_staples.core.storage import list_cached_tickers, get_mode
