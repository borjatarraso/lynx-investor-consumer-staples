"""Consumer-staples-focused report synthesis engine."""

from __future__ import annotations

import math

from lynx_staples.models import AnalysisConclusion, AnalysisReport, CompanyStage, CompanyTier, JurisdictionTier
...
# For consumer staples we weight profitability and business quality heavily,
# because defensive operators are cash-generative and brand/distribution
# driven rather than cash-survival driven. Mature staples earn premium
# weighting on profitability + quality given the dividend-funding profile.
# For early-stage / emerging staples brands we still emphasize solvency
# and dilution.
# ---------------------------------------------------------------------------
...
def _consumer_screening(r: AnalysisReport) -> dict:
    """10-point consumer-staples screening checklist."""
    ...
    # 4. Debt/EBITDA < 3x (manageable leverage for defensive sector)
...
def _tier_note(tier: CompanyTier) -> str:
    return {
        CompanyTier.MEGA: "Full traditional analysis. DCF, dividend coverage, and ROIC primary. Liquidity uniform.",
        CompanyTier.LARGE: "Full traditional analysis. Dividend Aristocrat tier — all metrics reliable.",
        CompanyTier.MID: "Blended analysis. Organic growth and margin trajectory weighted heavily.",
        CompanyTier.SMALL: "Balance sheet and brand strength critical. Distribution moat matters.",
        CompanyTier.MICRO: "Survival and working-capital efficiency dominate. Limited analyst coverage.",
        CompanyTier.NANO: "Speculative emerging staples brand. Asset-based valuation and cash position only.",
    }.get(tier, "")


def _stage_note(stage: CompanyStage) -> str:
    return {
        CompanyStage.PRODUCER: "Mature Defensive Operator: ROIC, operating margin, organic growth, FCF conversion, dividend coverage.",
        CompanyStage.DEVELOPER: "Scaling Staples Operator: distribution growth, shelf-space wins, CAPEX intensity, margin trajectory.",
        CompanyStage.EXPLORER: "Emerging Staples Brand: organic growth, gross margin, dilution, cash runway.",
        CompanyStage.GRASSROOTS: "Early Stage: cash runway, burn rate, founder ownership. Binary outcome risk.",
        CompanyStage.ROYALTY: "Brand Licensor / Asset-Light: royalty margins, licensee health, brand-equity maintenance, low capex.",
    }.get(stage, "")
