"""Interactive prompt-based mode for lynx-staples."""
...
from lynx_staples.core.news import download_article
from lynx_staples.core.reports import download_filing
from lynx_staples.display import display_full_report, display_report_stage
from lynx_staples.models import AnalysisReport

console = Console()

BANNER = "\n[bold blue]  L Y N X   Consumer Staples Analysis[/]\n[dim]    Consumer Staples Fundamental Analysis[/]\n"
...
def run_interactive():
    from lynx_staples.core.storage import get_mode, is_testing
    ...
    while True:
        ...
        try:
            console.print(f"\n[bold {prompt_color}]lynx-staples[/] ", end="")
            raw = input().strip()
        ...
        if cmd in ("quit", "exit", "q"):
            ...
            from lynx_staples import get_about_text; from rich.panel import Panel as P
            a = get_about_text()
            ...
            from lynx_staples.metrics.explanations import get_explanation
            ...
            from lynx_staples.metrics.explanations import list_metrics
            ...
            from lynx_staples.core.ticker import search_companies, display_search_results
            ...
            try:
                ...
                from lynx_staples.core.analyzer import SectorMismatchError
                if isinstance(e, SectorMismatchError):
                    console.print()
                    console.print(Panel(
                        f"[bold blink red]SECTOR MISMATCH — ANALYSIS BLOCKED[/]\n\n"
                        f"[bold red]{e}[/]\n\n"
                        f"[bold blink red]This tool is specialized ONLY for Consumer Staples.[/]",
            ...
            if current_report:
                from lynx_staples.display import _display_business_quality, _display_share_structure, _display_intrinsic_value, _display_market_intelligence
            ...
            if current_report and current_report.filings:
                from lynx_staples.display import _display_filings; _display_filings(current_report)
            else: console.print("[yellow]No filings available.[/]")
        elif cmd == "news":
            if current_report and current_report.news:
                from lynx_staples.display import _display_news; _display_news(current_report)
            ...
            from lynx_staples.core.storage import list_cached_tickers, get_mode
            ...
            from lynx_staples.core.storage import drop_cache_all, drop_cache_ticker, get_mode
            if arg.lower() == "all":
            ...
            from lynx_staples.metrics.explanations import get_section_explanation, SECTION_EXPLANATIONS
            if not arg:
            ...
            from lynx_staples.metrics.explanations import get_conclusion_explanation, CONCLUSION_METHODOLOGY
            ...
            from lynx_staples.easter import rich_matrix; rich_matrix(console)
        elif cmd == "fortune":
            from lynx_staples.easter import rich_fortune; rich_fortune(console)
        elif cmd == "rocket":
            from lynx_staples.easter import rich_rocket; rich_rocket(console)
        elif cmd == "lynx":
            from lynx_staples.easter import rich_lynx; rich_lynx(console)
        else:
