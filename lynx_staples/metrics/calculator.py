"""Consumer-staples-specialized metrics calculation engine.
...
def calc_efficiency(
    ...
    # Inventory turnover — critical for packaged food, beverages, food retail
...
# For consumer staples most segments are defensive. Agribusiness and, to a
# lesser extent, food/drug retail have modest cyclical sensitivity to
# commodity cycles and retailer margin pressure, respectively.
_MODERATELY_CYCLICAL_SEGMENTS = {
    Segment.AGRIBUSINESS,
}
_MILDLY_CYCLICAL_SEGMENTS = {
    Segment.FOOD_RETAIL, Segment.DRUG_RETAIL, Segment.HYPERMARKET,
}
...
def calc_business_quality(
    ...
    """Consumer-staples business-quality scoring.

    Weights five dimensions appropriate for defensive consumer operators:
    ...
    Plus a defensive-sensitivity qualitative tag (no score impact) that
    differentiates inelastic staples (tobacco, packaged food, household) from
    modestly-cyclical sub-segments (food/drug retail, agribusiness).
    """
    ...
    if gm is not None:
        if gm >= 0.50:
            m.brand_strength = "Premium brand — >50% gross margin signals durable pricing power (top CPG / spirits / tobacco)"
            m.brand_strength_score = 100.0
            score += 25
        elif gm >= 0.38:
            m.brand_strength = "Strong brand — 38-50% gross margin (typical branded CPG)"
            m.brand_strength_score = 75.0
            score += 18
        elif gm >= 0.25:
            m.brand_strength = "Moderate brand — 25-38% gross margin (packaged food / hypermarket-tier)"
            m.brand_strength_score = 50.0
            score += 10
        elif gm >= 0.15:
            m.brand_strength = "Thin margins — food-retail / private-label-exposed positioning"
            m.brand_strength_score = 25.0
            score += 4
        else:
            m.brand_strength = "Very thin margins — commodity-like positioning"
    ...
    try:
        ...
        from lynx_staples.models import classify_segment
    ...
    if primary_segment == Segment.TOBACCO:
        m.cyclical_sensitivity = "Very low — inelastic demand; ESG screen-out is the primary headwind"
    elif primary_segment in (Segment.PACKAGED_FOOD, Segment.BEVERAGES_NA, Segment.HOUSEHOLD_PRODUCTS, Segment.PERSONAL_PRODUCTS):
        m.cyclical_sensitivity = "Very low — defensive staple with inelastic demand"
    elif primary_segment == Segment.BEVERAGES_ALC:
        m.cyclical_sensitivity = "Low — modest premium-mix sensitivity to consumer confidence"
    elif primary_segment in _MILDLY_CYCLICAL_SEGMENTS:
        m.cyclical_sensitivity = "Low — defensive volumes, mild margin sensitivity to retailer pricing"
    elif primary_segment in _MODERATELY_CYCLICAL_SEGMENTS:
        m.cyclical_sensitivity = "Moderate — earnings tied to commodity / crush-margin cycles"
    else:
        m.cyclical_sensitivity = "Defensive sensitivity inferred from stage and leverage"

    # Channel mix quality proxy (asset-light brand vs capex-heavy retail) — inferred from capex intensity
    capex_int = growth.capex_intensity if growth else None
    if capex_int is not None:
        if capex_int < 0.03:
            m.channel_mix_quality = "Asset-light branded / licensing model"
        elif capex_int < 0.06:
            m.channel_mix_quality = "Balanced manufacturing + distribution mix"
        else:
            m.channel_mix_quality = "Capex-heavy retail / processing footprint"
    ...
    if tier in (CompanyTier.NANO, CompanyTier.MICRO):
        m.consumer_health_exposure = "Single-brand exposure — consumer health moves with the brand"
    elif primary_segment in _MODERATELY_CYCLICAL_SEGMENTS:
        m.consumer_health_exposure = "Cyclical — earnings sensitive to commodity-margin cycles"
    elif primary_segment == Segment.TOBACCO:
        m.consumer_health_exposure = "Inelastic — demand largely independent of consumer confidence"
    else:
        m.consumer_health_exposure = "Defensive exposure — staples spending insulated through cycles"
    ...
    if m.quality_score >= 75:
        m.competitive_position = "Strong Position — High-Quality Defensive Franchise"
    elif m.quality_score >= 55:
        m.competitive_position = "Viable Position — Moderate-Quality Staples Operator"
    elif m.quality_score >= 35:
        m.competitive_position = "Challenged — Below-Average Staples Quality"
    else:
        m.competitive_position = "High Risk — Weak Staples Fundamentals"
...
def calc_intrinsic_value(
    ...
    # Consumer-staples stage-appropriate valuation method selection
    if stage == CompanyStage.PRODUCER:
        iv.primary_method = "DCF (FCF to Firm) + Dividend Discount Model"
        iv.secondary_method = "EV/EBITDA Peer Comps"
    elif stage == CompanyStage.DEVELOPER:
        iv.primary_method = "EV/EBITDA Peer Comps"
        iv.secondary_method = "DCF with explicit distribution-expansion phase"
    elif stage == CompanyStage.EXPLORER:
...
def calc_market_intelligence(
    ...
    # --- Consumer staples macro & sector context ---
    # For staples operators the "commodity" slot is used for a defensive-demand
    # proxy (the broad consumer-staples sector ETF) rather than a physical
    # commodity. We also fetch a sub-segment peer ETF so investors can compare
    # relative defensive performance.
    _CONSUMER_PROXIES = {
        Segment.PACKAGED_FOOD:      [("PBJ", "Invesco Food & Beverage"), ("XLP", "Consumer Staples Select Sector SPDR")],
        Segment.BEVERAGES_NA:       [("PBJ", "Invesco Food & Beverage"), ("XLP", "Consumer Staples Select Sector SPDR")],
        Segment.BEVERAGES_ALC:      [("PBJ", "Invesco Food & Beverage"), ("XLP", "Consumer Staples Select Sector SPDR")],
        Segment.TOBACCO:            [("XLP", "Consumer Staples Select Sector SPDR"), ("KXI", "iShares Global Consumer Staples")],
        Segment.HOUSEHOLD_PRODUCTS: [("IYK", "iShares US Consumer Staples"), ("XLP", "Consumer Staples Select Sector SPDR")],
        Segment.PERSONAL_PRODUCTS:  [("IYK", "iShares US Consumer Staples"), ("XLP", "Consumer Staples Select Sector SPDR")],
        Segment.FOOD_RETAIL:        [("FTXG", "First Trust Nasdaq Food & Beverage"), ("XLP", "Consumer Staples Select Sector SPDR")],
        Segment.HYPERMARKET:        [("XRT", "SPDR S&P Retail"), ("XLP", "Consumer Staples Select Sector SPDR")],
        Segment.DRUG_RETAIL:        [("XLP", "Consumer Staples Select Sector SPDR"), ("XRT", "SPDR S&P Retail")],
        Segment.AGRIBUSINESS:       [("MOO", "VanEck Agribusiness"), ("XLP", "Consumer Staples Select Sector SPDR")],
        Segment.OTHER:              [("XLP", "Consumer Staples Select Sector SPDR"), ("VDC", "Vanguard Consumer Staples")],
    }
    ...
    # Fetch a broad defensive-demand proxy (Consumer Staples sector ETF)
    # stored in the legacy "commodity" slot for template parity across agents.
    try:
        demand_proxy = yf.Ticker("XLP")
        pi = demand_proxy.info or {}
        mi.commodity_name = "Consumer Staples (XLP)"
    ...
    if stage in pre_profit_stages and solvency and solvency.total_debt and solvency.total_debt > 0:
        warnings.append("Debt in pre-profit company — unusual for emerging staples brands")

    mi.risk_warnings = warnings

    # ── 8. Consumer-staples disclaimers ──────────────────────────────
    disclaimers: list[str] = [
        "Consumer staples demand is inelastic but volume can still erode through "
        "private-label substitution and category disruption (e.g. GLP-1 drugs).",
        "Pricing power has carried recent organic growth — watch for the volume / "
        "price split as inflation moderates.",
        "Input-cost inflation (commodities, packaging, energy, labour) compresses "
        "gross margins faster than pricing can typically catch up.",
    ]
    if stage in (CompanyStage.GRASSROOTS, CompanyStage.EXPLORER):
        disclaimers.append(
            "This company is an emerging staples brand. Distribution, shelf-space, "
            "and brand durability are unproven; many challenger brands fail before "
            "reaching scale or are acquired by incumbents."
        )
    if stage == CompanyStage.DEVELOPER:
        disclaimers.append(
            "Rapid distribution expansion often stretches unit economics. Watch "
            "trade-spend trends and gross-to-net adjustments closely."
        )
    if stage == CompanyStage.ROYALTY:
        disclaimers.append(
            "Brand-licensing models depend on licensee health and brand-equity "
            "maintenance; weakening licensees can compress royalty income."
        )
    disclaimers.extend([
