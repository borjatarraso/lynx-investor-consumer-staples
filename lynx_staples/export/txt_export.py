...
from lynx_staples.models import AnalysisReport, CompanyStage, Severity
...
def export_txt(report: AnalysisReport, output_path: Path) -> Path:
    ...
    lines += _header(f"LYNX Consumer Staples  --  {p.name} ({p.ticker})")
    ...
    if c.screening_checklist:
        lines.append("")
        lines.append("  Consumer Staples Screening Checklist:")
        lines.append("  " + "-" * 40)
    ...
    lines.append(f"  Lynx Consumer Staples (Lince Investor Suite)")
    lines.append("=" * W)
