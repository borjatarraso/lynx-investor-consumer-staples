*** Settings ***
Documentation    Export workflow tests for lynx-staples (uses Python API to avoid network timeouts)
Library          Process
...
Export TXT From Minimal Report
    ...
    When I Run Python Code "from lynx_staples.models import AnalysisReport, CompanyProfile, CompanyTier, CompanyStage, ValuationMetrics, SolvencyMetrics; from lynx_staples.export import export_report, ExportFormat; from pathlib import Path; import tempfile; p = CompanyProfile(ticker='TEST', name='Test Staples Corp', sector='Consumer Defensive', industry='Packaged Foods', country='United States', market_cap=100000000); p.tier = CompanyTier.MICRO; p.stage = CompanyStage.EXPLORER; r = AnalysisReport(profile=p, valuation=ValuationMetrics(pb_ratio=0.8, cash_to_market_cap=0.35), solvency=SolvencyMetrics(cash_runway_years=3.0, total_cash=35000000)); f = tempfile.NamedTemporaryFile(suffix='.txt', delete=False); path = export_report(r, ExportFormat.TXT, Path(f.name)); content = path.read_text(); assert 'Test Staples Corp' in content; assert 'Emerging' in content; assert len(content) > 500; print('OK', len(content), 'bytes')"
...
Export HTML From Minimal Report
    ...
    When I Run Python Code "from lynx_staples.models import AnalysisReport, CompanyProfile, CompanyTier, CompanyStage, ValuationMetrics, SolvencyMetrics; from lynx_staples.export import export_report, ExportFormat; from pathlib import Path; import tempfile; p = CompanyProfile(ticker='TEST', name='Test Staples Corp', sector='Consumer Defensive', industry='Packaged Foods', country='United States', market_cap=100000000); p.tier = CompanyTier.MICRO; p.stage = CompanyStage.EXPLORER; r = AnalysisReport(profile=p, valuation=ValuationMetrics(pb_ratio=0.8, cash_to_market_cap=0.35), solvency=SolvencyMetrics(cash_runway_years=3.0, total_cash=35000000)); f = tempfile.NamedTemporaryFile(suffix='.html', delete=False); path = export_report(r, ExportFormat.HTML, Path(f.name)); content = path.read_text(); assert '#fff' in content or '#ffffff' in content, 'Missing white background'; assert 'word-wrap' in content, 'Missing word-wrap'; assert 'Test Staples Corp' in content; assert len(content) > 1000; print('OK', len(content), 'bytes')"
    Then The Exit Code Should Be 0
    Then The Output Should Contain "OK"

HTML Has No Text Truncation
    [Documentation]    GIVEN an HTML export WHEN I check table cells THEN word-wrap is enabled
    When I Run Python Code "from lynx_staples.models import AnalysisReport, CompanyProfile, CompanyTier, CompanyStage, ValuationMetrics; from lynx_staples.export import export_report, ExportFormat; from pathlib import Path; import tempfile; p = CompanyProfile(ticker='T', name='T Corp'); p.tier = CompanyTier.MICRO; p.stage = CompanyStage.EXPLORER; r = AnalysisReport(profile=p, valuation=ValuationMetrics(pb_ratio=0.8)); f = tempfile.NamedTemporaryFile(suffix='.html', delete=False); path = export_report(r, ExportFormat.HTML, Path(f.name)); c = path.read_text(); assert 'overflow-wrap' in c and 'break-word' in c; assert 'table-layout' in c and 'fixed' in c; print('OK')"
    Then The Exit Code Should Be 0
    Then The Output Should Contain "OK"
